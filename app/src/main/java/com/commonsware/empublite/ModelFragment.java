...
public class ModelFragment extends SherlockFragment {
    ...
    /*
        FROM BOOK p464:
            Here, we call getDefaultSharedPreferences() in doInBackground(). We also call
            getAll() on the SharedPreferences object, to make sure that it is fully loaded from
            disk, in case Android has an optimization that lazy-loads the preference data on first
            use. In onPostExecute(), we store the resulting SharedPreferences in a data
            member, clear our prefsTask data member (indicating that we are done with the
    ...
    private class ContentsLoadTask extends AsyncTask<Context, Void, Void> {
        ...
        protected Void doInBackground(Context... ctxt) {
            ...
            try {
                ...
                if (updateDir != null && new File(updateDir).exists()) {
                    localContents=
                            new BookContents(new JSONObject(buf.toString()), new File(updateDir));
                }
                else {
                    localContents=
